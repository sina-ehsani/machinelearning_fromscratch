...
class Dataset(object):
    ...
    def convert_labels_to_numerical(self, positive_label):
        
        def binarize_label(labeled_sample):
            return LabeledSample(
                1 if labeled_sample.label == positive_label else -1,
                labeled_sample.features)

        new_tr = list(binarize_label(ls) for ls in self.training_set)
        new_v = list(binarize_label(ls) for ls in self.validation_set)
        new_te = list(binarize_label(ls) for ls in self.testing_set)
        try:
            new_md = DatasetMetadata(self.metadata.feature_values,
                                     set([-1, 1]))
        except AttributeError:
            # don't convert metadata when it's already numerical
            new_md = self.metadata
        result = Dataset(new_tr, new_v, new_te, new_md)
        result.is_numerical = self.is_numerical
        return result
    
    def convert_features_to_numerical(self, add_bias=False):
        ...
        try:
            ...
            new_md.columns = list((f, feature_values[f]) for f in features)# feature_values
            column_names = []
            for f, fvs in new_md.columns:
                for v in fvs:
                    column_names.append(f + "-" + v)
            new_md.column_names = column_names
        except AttributeError:
            # don't convert metadata when it's already numerical
            new_md = self.metadata

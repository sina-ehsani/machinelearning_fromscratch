...
class DecisionTreeClassifier:
    ...
    def entrupy_cal(self,dictionery):
        import math
        entrupy = 0
        size = sum(dictionery.values())
        for value in dictionery.values():
            percent = value/size
            entrupy -= (percent*math.log(percent,2))
        return entrupy
    ...
    def _best_split(self, dataset):
        ...
        total_ent=self.entrupy_cal(num_samples_per_class)
        
        self.score_dict=dict()
        for feat in self.features_.keys():
            self.score_dict[feat]=total_ent
            for value in self.features_[feat]:
                ...
                if sub_class_dict: 
                    ent_value = (sum(sub_class_dict.values())/totaldata) * self.entrupy_cal(sub_class_dict)
                else: ent_value=0
                self.score_dict[feat] -= ent_value
